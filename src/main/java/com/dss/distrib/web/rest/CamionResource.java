...
/**
 * REST controller for managing {@link com.dss.distrib.domain.Camion}.
 */
@RestController
@RequestMapping("/api")
public class CamionResource {
     ...
     */
    @PostMapping("/camions")
    public ResponseEntity<Camion> createCamion(@Valid @RequestBody Camion camion) throws URISyntaxException {
    ...
    public ResponseEntity<Camion> updateCamion(
        ...
        @Valid @RequestBody Camion camion
    ...
    public ResponseEntity<Camion> partialUpdateCamion(
        @PathVariable(value = "id", required = false) final Long id,
        @NotNull @RequestBody Camion camion
    ) throws URISyntaxException {
     ...
     */
    @GetMapping("/camions")
    public ResponseEntity<List<Camion>> getAllCamions(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of Camions");
        Page<Camion> page = camionService.findAll(pageable);
